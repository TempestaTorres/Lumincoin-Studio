...
class CategoryIncomeController {
    static getCategories(req, res) {
        ...
        res.json(operations.map(item => ({id: item.id, title: item.title, color: item.color})));
    }
    ...
    static createCategory(req, res) {
        ...
        let category = CategoryIncomeModel.findOne({title: title, color: color, user_id: req.body.user.id});
        if (category) {
            return res.status(400)
    ...
    static updateCategory(req, res) {
        ...
        res.json(CategoryIncomeModel.update({id: parseInt(id), user_id: req.body.user.id}, title, color));
    }
